...
function makeRoadSegment(THREE, theme, z) {
  ...
  const road = new THREE.Mesh(new THREE.BoxGeometry(10.4, 0.08, 46), roadMat);
...
function makeTunnel(THREE) {
  ...
  const roof = new THREE.Mesh(new THREE.BoxGeometry(12.8, 0.74, TUNNEL_LENGTH), concrete);
  roof.position.y = 8.4;
  ...
  for (const z of [-TUNNEL_HALF_LENGTH + 0.4, TUNNEL_HALF_LENGTH - 0.4]) {
    const header = new THREE.Mesh(new THREE.BoxGeometry(13.3, 0.6, 0.7), concrete);
    header.position.set(0, 8.65, z);
    group.add(header);
...
function makeTrafficLight(THREE) {
  ...
  const stopLine = new THREE.Mesh(new THREE.BoxGeometry(8.8, 0.08, 0.42), lineMat);
  stopLine.position.set(0, 0.16, STOP_LINE_OFFSET);
...
export default function Page() {
  ...
  const keysRef = useRef({ left: false, right: false, forward: false, brake: false });
  ...
  const [runHud, setRunHud] = useState({ fuel: 100, coins: 0, distance: 0, speed: 0, inTunnel: false });
  ...
  useEffect(() => {
    ...
    async function boot() {
      ...
      for (let i = 0; i < 50; i += 1) {
        ...
        tree.position.set(side * (9 + Math.random() * 20), 0, -Math.random() * 260);
      ...
      const run = {
        ...
        distance: 0,
        coins: 0,
        speed: 34 + stats.acceleration * 22,
        spawnTimer: 0.8,
        difficulty: 0,
        inTunnel: false,
        signalTime: 0,
      ...
      function animate(now) {
        ...
        const normalTargetSpeed = 34 + run.stats.acceleration * 28 + (keysRef.current.forward ? 13 : 0) + run.difficulty * 5.2;
        const targetSpeed = keysRef.current.brake ? 0 : normalTargetSpeed;
        run.speed += (targetSpeed - run.speed) * dt * (keysRef.current.brake ? 5.8 : 2.6);
        if (keysRef.current.brake && run.speed < 0.8) run.speed = 0;
        run.audio?.setSpeed(run.speed, keysRef.current.forward);
        ...
        for (let i = run.objects.length - 1; i >= 0; i -= 1) {
          ...
          if (item.kind === "traffic") {
            const activeLight = run.trafficLights.find(
              (light) => {
                const stopLineZ = light.position.z + STOP_LINE_OFFSET;
                return stopLineZ > item.mesh.position.z - 8 && stopLineZ - item.mesh.position.z < 118;
              }
            );
          ...
          if (shouldStop && trafficStopLineZ !== null && item.mesh.position.z > trafficStopLineZ - 4.5) {
            item.mesh.position.z = trafficStopLineZ - 4.5;
            item.trafficSpeed = 0;
          }
  ...
  useEffect(() => {
    function down(event) {
      ...
      if ((key === "a" || key === "arrowleft") && run) {
        run.targetLane = clamp(run.targetLane - 1, 0, 2);
        event.preventDefault();
      }
      if ((key === "d" || key === "arrowright") && run) {
        run.targetLane = clamp(run.targetLane + 1, 0, 2);
        event.preventDefault();
      }
    ...
    function up(event) {
      ...
      if (key === "s" || key === "arrowdown") keysRef.current.brake = false;
    }
  ...
  function startRun(mapId = profile.selectedMap) {
    ...
    setResult(null);
    setRunHud({ fuel: 100, coins: 0, distance: 0, speed: 0, inTunnel: false });
    setScreen("game");
  }
